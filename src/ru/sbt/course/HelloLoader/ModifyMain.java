...
//        Ищем уже в корректном классе метод sayHello() и если такой метод есть, вызываем его. Победа!
        ...
        //проверка работы HelloClassloader
        // добавлено Hin7 13/05/2020
        System.out.println("\nЗагружаем класс sayHello с помощью своего класслоадера");
        //Class<?> cl = new HelloClassloader(DIR_NAME_OLD, DIR_NAME_NEW, METHOD_NAME_CORRECT,
        //        ClassLoader.getSystemClassLoader()).loadClass(CLASS_NAME);
        Class<?> cl = new HelloClassloader2(DIR_NAME_OLD, DIR_NAME_NEW, METHOD_NAME_CORRECT,
                ClassLoader.getSystemClassLoader()).loadClass(CLASS_NAME);
        Method method = cl.getDeclaredMethod(METHOD_NAME_CORRECT);
        System.out.println("Вызываем метод " + method.getName() + " из класса " + cl.getName() +
                " загруженного класслоадером " + cl.getClassLoader());
